...
export interface InventoryAvailabilityPort { getOnlineSellableQuantity(variantId:string):Promise<number>; getOnlineSellableQuantities(variantIds:string[]):Promise<Record<string,number>>; getUnitCostToman(variantId:string):Promise<number|null>; planOnlineReservation(items:{variant_id:string;quantity:number}[]):Promise<{warehouse_id:string;variant_id:string;quantity:number}[]>; }
