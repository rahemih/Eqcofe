...
export class ProductQaService {
  ...
  async ask(input: { productSlug: unknown; question: unknown }) {
    ...
    if (!answer || answer.length > MAX_ANSWER_LENGTH) {
      throw new DomainError('AI_PRODUCT_QA_RESPONSE_INVALID', 'پاسخ تولیدشده معتبر نیست.');
    }

    return {
      answer,
      product: { slug: product.slug, name_fa: product.name_fa },
  ...
  private authoritativeContext(product: any) {
    const context = {
      ...
      specifications: Array.isArray(product.specifications)
        ? product.specifications.slice(0, 80).map((item: any) => this.fact(item)).filter(Boolean)
        : [],
      variants: Array.isArray(product.variants)
        ? product.variants.slice(0, 40).map((variant: any) => ({
            name_suffix: this.optionalString(variant.name_suffix ?? variant.nameSuffix, 200),
            attributes: Array.isArray(variant.attributes)
              ? variant.attributes.slice(0, 30).map((item: any) => this.fact(item)).filter(Boolean)
              : [],
          }))
        : [],
    };
  ...
  private question(value: unknown): string {
    const question = String(value ?? '').trim();
    if (question.length < 2 || question.length > MAX_QUESTION_LENGTH) throw new DomainError('AI_PRODUCT_QA_QUESTION_INVALID', 'پرسش باید بین ۲ تا ۱۰۰۰ نویسه باشد.');
    return question;
  }
