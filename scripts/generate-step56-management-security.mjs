...
const input='docs/13-product-design/step56-management-security-wireframes.json';
const root='docs/13-product-design/step56-wireframes/G';
...
function frame(s,v,w){
 ...
 if(v.kind==='dialog'){body+=box(x,y,width,48,v.actionDisabled?'#eeeeee':'#ffffff')+text(v.action,x+width-16,y+30);y+=70;}
...
files[`${root}/README.md`]=`# Step 56-G — Finance, Analytics, Configuration and Security\n\n34 obligations, ${frameCount} low-fidelity RTL SVG frames. Only G is designed; H remains NOT_STARTED. A is an immutable foundation snapshot; the G contract records current progression. Open implementation evidence remains NO_ACTION.\n\nView gallery.html for a local review index; no network, Figma or paid service is required.\n\nOperations screens inherit the B shell. All185 G operations are traced;94 remain NO_ACTION, including read operations. H is not started.\n`;
...
files[`${root}/gallery.html`]=`<!doctype html><html lang="fa" dir="rtl"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>EQCOFE — Step 56-G</title><style>body{font:16px/1.8 Tahoma,Arial,sans-serif;background:#eee;color:#111;margin:24px}h1,h2{overflow-wrap:anywhere}article{background:white;padding:16px;margin:24px 0;border:1px solid #777}img{display:block;max-width:100%;height:auto;margin-top:16px}a{display:inline-block;min-height:44px}:focus-visible{outline:3px solid #111;outline-offset:4px}</style><h1>مالی، تنظیمات و امنیت — مرحلهٔ 56-G</h1><p>این صفحه فهرست شواهد طراحی است؛ محصول اجرایی نیست. تصاویر کوچک‌شده را برای بررسی دقیق در اندازهٔ اصلی باز کنید.</p>${cards}</html>\n`;
files[`${root}/manifest.json`]=json({step:56,substep:'G',source:input,sourceSha256:hash(raw),screenCount:c.screens.length,frameCount,artifacts:Object.entries(files).map(([path,content])=>({path,sha256:hash(content),bytes:Buffer.byteLength(content)}))});
function walk(p){try{return readdirSync(p,{withFileTypes:true}).flatMap(e=>e.isDirectory()?walk(p+'/'+e.name):[p+'/'+e.name]);}catch{return [];}}
if(process.argv.includes('--check')){const errors=[];for(const [p,v] of Object.entries(files)){try{if(readFileSync(p,'utf8').replaceAll('\r\n','\n')!==v)errors.push(p);}catch{errors.push(p);}}errors.push(...walk(root).filter(p=>!(p in files)));if(errors.length)throw Error('G artifact drift: '+errors.join(', '));}
else for(const [p,v] of Object.entries(files)){mkdirSync(dirname(p),{recursive:true});writeFileSync(p,v);}
console.log(`Step 56-G: PASS; ${frameCount} frames; ${Object.keys(files).length} artifacts`);
