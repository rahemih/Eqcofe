...
const input='docs/13-product-design/step56-catalog-media-wireframes.json';
const root='docs/13-product-design/step56-wireframes/C';
...
files[`${root}/README.md`]=`# Step 56-C — Catalog and Media\n\n8 obligations, ${frameCount} low-fidelity RTL SVG frames. Only C is designed; D–H remain NOT_STARTED. A is an immutable foundation snapshot; the C contract records current progression. Open implementation evidence remains NO_ACTION.\n\nView gallery.html for a local review index; no network, Figma or paid service is required.\n\nCatalog/media screens inherit the B shell. All 46 operations and source-specific lifecycle, upload, assignment, version and sales-preview constraints are traced. D–H are not started.\n`;
...
files[`${root}/gallery.html`]=`<!doctype html><html lang="fa" dir="rtl"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>EQCOFE — Step 56-C</title><style>body{font:16px/1.8 Tahoma,Arial,sans-serif;background:#eee;color:#111;margin:24px}h1,h2{overflow-wrap:anywhere}article{background:white;padding:16px;margin:24px 0;border:1px solid #777}img{display:block;max-width:100%;height:auto;margin-top:16px}a{display:inline-block;min-height:44px}:focus-visible{outline:3px solid #111;outline-offset:4px}</style><h1>کاتالوگ و رسانهٔ مدیریت — مرحلهٔ 56-C</h1><p>این صفحه فهرست شواهد طراحی است؛ محصول اجرایی نیست. تصاویر کوچک‌شده را برای بررسی دقیق در اندازهٔ اصلی باز کنید.</p>${cards}</html>\n`;
files[`${root}/manifest.json`]=json({step:56,substep:'C',source:input,sourceSha256:hash(raw),screenCount:c.screens.length,frameCount,artifacts:Object.entries(files).map(([path,content])=>({path,sha256:hash(content),bytes:Buffer.byteLength(content)}))});
function walk(p){try{return readdirSync(p,{withFileTypes:true}).flatMap(e=>e.isDirectory()?walk(p+'/'+e.name):[p+'/'+e.name]);}catch{return [];}}
if(process.argv.includes('--check')){const errors=[];for(const [p,v] of Object.entries(files)){try{if(readFileSync(p,'utf8').replaceAll('\r\n','\n')!==v)errors.push(p);}catch{errors.push(p);}}errors.push(...walk(root).filter(p=>!(p in files)));if(errors.length)throw Error('C artifact drift: '+errors.join(', '));}
else for(const [p,v] of Object.entries(files)){mkdirSync(dirname(p),{recursive:true});writeFileSync(p,v);}
console.log(`Step 56-C: PASS; ${frameCount} frames; ${Object.keys(files).length} artifacts`);
