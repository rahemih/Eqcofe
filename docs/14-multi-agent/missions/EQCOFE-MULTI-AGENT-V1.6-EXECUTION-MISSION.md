# EQCOFE — FINAL EXECUTION MISSION V1.6
...
Mission:
EQCOFE-MULTI-AGENT-V1.6-EXECUTION-MISSION.md

Registry:
REGISTRY.md

Acceptance Closure:
EQCOFE-MULTI-AGENT-V1.6-ACCEPTANCE-CLOSURE.md
...
The Acceptance Closure is a two-part canonical record:

```text
A. Static Closure Record
   = repository file merged through the canonical protected path

B. Terminal Provider Seal
   = immutable GitHub provider evidence recorded only after
     the Closure PR merge and exact-SHA post-merge verification
```

The Static Closure Record must be stored in the Canonical missions directory.
...
## 32.1 Static Closure Record

The Static Closure Record is created before protected merge and may contain only facts that are already knowable and independently verifiable at that time.
...
The Static Closure Record MUST NOT guess, predict, precompute, or self-embed terminal facts that do not yet exist.

Therefore, before merge it MUST NOT claim its own final:
...
A placeholder, guessed identifier, circular fixed-point construction, or narrative assertion for any of those fields is prohibited.

## 32.2 Terminal Provider Seal

Only after:

```text
Protected Closure Merge = PASS
...
the merged Closure PR must receive a Terminal Provider Seal.

The Seal is an immutable GitHub evidence record that indexes Level 1 provider facts and Level 2 deterministic controller results. The Seal itself cannot override or replace those facts.
...
Final Live Revalidation Result

Final Live Revalidation UTC Timestamp
...
Every terminal identifier must be read from GitHub/provider facts after it exists.

## 32.3 Canonical Closure Rule
...
is present in the exact Closure Merge SHA

Protected Closure Merge = PASS

Exact-SHA Post-Merge = PASS

Final Live Revalidation = PASS
within the Section 35 time bound

Terminal Provider Seal
is bound to the exact Closure Task / PR / Head / Merge

Residual State Check = PASS

No mandatory acceptance blocker remains
```

The repository file and the Terminal Provider Seal together form the canonical Acceptance Closure Record.

The Terminal Provider Seal resolves the unavoidable temporal boundary between pre-merge repository bytes and post-merge provider facts. Terminal facts MUST NOT be fabricated inside the pre-merge file.

After Canonical Closure:

```text
Direct modification of the Static Closure Record
= PROHIBITED
```

Any correction requires a governed follow-up closure correction or Mission amendment with fresh artifact-bound gates.
