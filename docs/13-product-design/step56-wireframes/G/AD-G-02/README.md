# AD-G-02 — دفتر و ثبت و برگشت سند

2..500 one-sided positive debit/credit lines with equal safe-integer totals, active postable accounts; occurred_at<=now+5min. Post draft only with reason; reverse posted once via compensating posted entry. All actions retain frozen operation authority; no additional runtime or security guarantee is introduced.
