...
test('A9 retains all Step 48 focused regression suites A2 through A8',()=>{const files=new Set(readdirSync('test'));for(const name of ['ai-provider-contracts-a2.spec.ts','ai-governed-prompts-a3.spec.ts','ai-provider-adapter-a4.spec.ts','ai-product-qa-a5.spec.ts','ai-draft-content-a6.spec.ts','ai-usage-controls-a7.spec.ts','ai-observability-a8.spec.ts'])assert.equal(files.has(name),true,name);});
