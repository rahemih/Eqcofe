...
test('Step 09 search returns priced public read models with cursor pagination',async()=>{
  ...
  assert.equal(result.items[0].price.current_toman,1000);
...
test('Step 09 suggestions are safe for blank input and bounded for populated input',async()=>{
  ...
  const result=await query.suggestions({q:' آسیاب ',limit:20});
  assert.equal(calls,1);
  assert.equal(result.suggestions[0].slug,'grinder');
});

test('Step 09 explicit out-of-range public limits fail closed',async()=>{
  const query=service();
  await assert.rejects(()=>query.search({q:'تمپر',limit:101}),/محدوده باید عدد صحیح/);
  await assert.rejects(()=>query.suggestions({q:'آسیاب',limit:21}),/محدوده باید عدد صحیح/);
});
