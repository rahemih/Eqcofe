...
const task = JSON.parse(readFileSync(new URL('../../docs/14-multi-agent/tasks/MA-V1-ACC-FAILCLOSED-5C-R2-001.json', import.meta.url), 'utf8'));
...
test('5C R2: declared paths accepted; actual undeclared fixture rejected', () => {
  assert.deepEqual(validate(allowed), [...allowed].sort());
  assert.equal(readFileSync(new URL('../../' + fixture, import.meta.url), 'utf8'), 'TEST_ONLY: undeclared inert fixture for Test 5C R2. NEVER MERGE.\n');
  assert.throws(() => validate([...allowed, fixture]), { message: 'OUT_OF_SCOPE:' + fixture });
});

test('5C R2: canonical pre-write validation blocks the writer callback', () => {
  let writes = 0;
...
test('5C R2: canonical lock is limited to declared scope and releases only on terminal abort', () => {
...
test('5C R2: real artifact bytes produce the exact scope merge blocker', () => {
