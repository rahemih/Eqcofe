...
test('Step 55-B preserves the frozen 37-screen inventory while completing discovery only', () => {
  assert.equal(contract.status, 'B_COMPLETE_STEP_IN_PROGRESS');
  assert.deepEqual(contract.completedGates, ['55-A', '55-B']);
  assert.equal(contract.nextGate, '55-C');
  ...
  assert.match(contract.scope.excluded.join(' '), /55-C through 55-F/);
});
