...
test('Step 55-A freezes 37 screens across five later gates without starting 55-B', () => {
  assert.equal(contract.status, 'A_COMPLETE_STEP_IN_PROGRESS');
  ...
  assert.match(contract.scope.excluded.join(' '), /55-B through 55-F/);
});
